...
class _Constants(object):
    ...
    '''
    _HEADER = {'User-Agent':
              'CORGIS Tate library for educational purposes'}
...
def get_artwork(test=False):
    """
    Returns a list of all the pieces of art in the Tate collection.

    """
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM tate LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM tate".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
...
def _test_interfaces():
    ...
    # Test test
    print("Test get_artwork")
    start_time = _default_timer()
    result = get_artwork(test=True)
